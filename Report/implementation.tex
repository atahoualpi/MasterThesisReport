...
\section{Modal data}
Using the peak locator algorithm described in section \ref{sec:chuck}, we extracted the modal data - frequencies and their corresponding amplitudes - from the recordings. Figure \ref{fig:cup_data} shows the data extracted for the cup object. Each graph corresponds to one of the ``sound areas'' we have divide it into.

Although, theoretically, each object can be modeled using one vector of frequencies and multiple vectors of amplitude data one for each different point area, we can see in figure \ref{fig:cup_data} that this does not happen in practice. However, the cause of the different resonant frequencies per area could be the environment noise, the difference in the action of hitting the object or the way it is hold when being hit.
...
Nevertheless, it is obvious that the fundamental frequencies are similar for each graph and lay just below 3000Hz.
